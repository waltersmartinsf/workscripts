...
%to reduce data
addpath('/Users/walterwsmf/Dropbox/research/ica_project/lightcurve/pipeline/')

%let's import our data
%change for the new directory
cd('/Users/walterwsmf/Dropbox/research/ica_project/lightcurve/')
%importing csv file remove the first row and first column (index values)
X = csvread('synth_observations_1.csv',1,1)
...
saveas(gca,'synth_observations_example_mixing_signals','png')

%Applied ICA-clustering in the data:
[result, ISRefica, ISRwasobi, Wmatrix, Wefica, Wwasobi, nonseparablecomponents] = icapy(X)
...
saveas(gca,'synth_observations_example_original_signals_result','png')

% figure()
% plot(signal','DisplayName','signal')
% title('Reulsts signals rescaled from ICA')
% saveas(gca,'synth_observations_example_original_signals_scaled','png')
...
colorbar
saveas(gca,'synth_observations_example_ISR_normalized_efica','png')
...
colorbar
saveas(gca,'synth_observations_example_ISR_normalized_wasobi','png')

%comparing initial and final lightcurves


%Saving our results
csvwrite('synth_observations_example_resuts.csv',result)

%save matlab workspace for future reference
save('synth_observations_example.mat')